...
MJZ_CX_FN static auto vformat(basic_string_view_t<version_v> fmt,
  ...
  auto opt = print_t<version_v>::vformat_to(ret,fmt, std::forward<Ts>(args)...);
  if (!opt) {
...
MJZ_CX_FN static auto formatln(auto fmt, Ts&&... args) noexcept {
  constexpr version_t version_v{std::remove_cvref_t <decltype(fmt())>::Version_v};

  basic_str_t<version_v, false> ret{};
  auto opt = print_t<version_v>::formatln_to(ret, fmt, std::forward<Ts>(args)...);
  if(!opt) {
    ret = basic_str_t<version_v, false>(unsafe_ns::unsafe_v, opt());
  }
      return ret;
