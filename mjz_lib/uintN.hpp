...
public:
    ...
    {
        using ret_t = pair_t<uintN_t<version_v,((word_count+1) / 2) * 64>,
            uintN_t<version_v,((word_count) / 2) * 64>>;
    ...
    MJZ_CX_AL_FN uintlen_t n_bit() const noexcept {
        return words.size() * 64;
    }
    MJZ_CX_AL_FN uintlen_t n_word() const noexcept {
        return words.size();
    }
    ...
    MJZ_CX_AL_FN uintN_t rotl(intlen_t r) const noexcept {
        return rotr(-r);
    }
    ...
    MJZ_CX_AL_FN uintN_t bitswap() const noexcept
    {
        return  bitswap(*this);
    }
    ...
    template<uintlen_t n>
    MJZ_CX_AL_FN uintn_t_<n_bits*n> pow(){
        if constexpr(n==1){
            return *this;
        } else if(n==2){
            return operator_mul_ret(*this);
        } else{
            return uintn_t_<n_bits*n>(pow<n-(n/2)>().operator_mul_ret(pow<n/2>()));
        }
    }
    ...
    MJZ_CX_AL_FN uintN_t &operator--() noexcept {
        return *this -= 1;
    }
...
template <version_t version_v> struct vectorizable_conv_base_impl_t {
    struct convert_base10p8_to_base10_u64_t {
    private:
        constexpr static inline uint64_t mask = (uint64_t(1) << 47) - 1;
        constexpr static inline uint64_t inv10p6_47b = 140737489;
        constexpr static inline uint64_t mask_upper_6b = 0xfc00'fc00'fc00'fc00;
        constexpr static inline uint64_t inv10p1_b10 = 103;
        constexpr static inline uint64_t ascii_offset = details_ns::ascii_offset;

    public:
        MJZ_CX_FN uint64_t operator()(const uint64_t less10p8) const noexcept {
            return  call(uint32_t(less10p8));
        }
        MJZ_CX_FN uint64_t operator()(const uint32_t less10p8) const noexcept {
            return call(less10p8);
        }
        MJZ_CX_FN static uint64_t call(const uint32_t less10p8)   noexcept {
            uint64_t val = less10p8;
            ...
            return val | ascii_offset;
        }
    };
    struct convert_base10_to_base10p8_u64_t {
    private:
        constexpr static inline uint64_t high16_half_mask = 0xff00ff00ff00ff00;

        constexpr static inline uint64_t high32_half_mask = 0xffff0000ffff0000;

        constexpr static inline uint32_t high8_half_mask32 = uint32_t(0xf0f0f0f0);

        constexpr static inline uint32_t high16_half_mask32 =
            uint32_t(0xff00ff00);

        constexpr static inline uint32_t high32_half_mask32 =
            uint32_t(0xffff0000);
    ...
    public:
        MJZ_CX_FN static uint32_t call(uint64_t ascii) noexcept {
          ...
          auto [r7, r6, r5, r4, r3, r2, r1, r0] =
            ...
            val -= ((val & (high32_half_mask32)) >> 12) * 3471;
            return val;
        }
        MJZ_CX_FN uint32_t operator()(const uint64_t ascii) const noexcept {
            return call(ascii);
        }
    };

    constexpr static inline convert_base10_to_base10p8_u64_t b10p1_2_10p8{};
    constexpr static inline convert_base10p8_to_base10_u64_t b10p8_2_10p1{};
    template <std::ranges::input_range R = std::span<uint64_t>>
        requires std::same_as<std::ranges::range_reference_t<R>,uint64_t &>
    MJZ_CX_FN static void base10p8_to_base10_u64(R &&vals) noexcept {
        for(uint64_t &elem : std::forward<R>(vals)) {
            asserts(asserts.assume_rn,elem < 100000000ull);
            elem = b10p8_2_10p1(elem);
        }
    }
    template <std::ranges::input_range R = std::span<uint64_t>>
        requires std::same_as<std::ranges::range_reference_t<R>,uint64_t &>
    MJZ_CX_FN static void base10_to_base10p8_u64(R &&vals) noexcept {
        for(uint64_t &elem : vals)
            elem = b10p1_2_10p8(elem);
    }
    constexpr static inline auto b10p1_2_10p8_view =
        std::views::transform(b10p1_2_10p8);
    constexpr static inline auto b10p8_2_10p1_view =
        std::views::transform(b10p8_2_10p1);
};

template <version_t version_v>
constexpr static inline vectorizable_conv_base_impl_t<version_v>
vectorizable_conv_base_impl{};

MJZ_CX_FN void
memcpy_u64_to_char_buffer(std::span<char> out,
                          const uint64_t *MJZ_restrict vals) noexcept {
    MJZ_IFN_CONSTEVAL{
